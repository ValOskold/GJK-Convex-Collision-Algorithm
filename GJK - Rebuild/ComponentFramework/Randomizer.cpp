...
float Randomizer::rand(const float low, float high) {
	return static_cast<float>((ran1()*(high - low)) + low);
}

int Randomizer::rand(const int low, int high){
	return static_cast<int>((ran1()*(static_cast<double>(high) - static_cast<double>(low))) + static_cast<double>(low));
}



/// Based on the work of Stephen K. Park and Keith W. Miller
/// Communications of the ACM, Oct 1988, Vol 31 Num 10
